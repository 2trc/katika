...
logging.basicConfig(filename= os.path.join(os.getcwd(), "logs","get_armp_entries.log"),
...
def avis_to_armp_entry(avis):
    ...
    try:
        ...
        logging.error("Owner list needs to be updated with short name: %s", avis.get('owner_short'))
        logging.exception(e)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        logging.info("Running with start date [%s], end date [%s]",
        ...
        while not stop_flag:
            ...
            for entry in entries:
                ...
                if start_date <= tender.publication_datetime.date() <= end_date:
                    try:
                        tender.save()

                        save_count += 1
                    except Exception as e:
                        logging.warning('Problem while logging, probably a duplicate. Entry: %s', entry)
                        logging.exception(e)
            ...
            logging.info("%d entries saved", save_count)

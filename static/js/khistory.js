var khistoryApp = angular.module('khistoryApp', ["ngRoute"
]);

//https://stackoverflow.com/questions/41211875/angularjs-1-6-0-latest-now-routes-not-working
khistoryApp.config(['$locationProvider', '$httpProvider', function($locationProvider,$httpProvider) {
    $locationProvider.hashPrefix('');

}]);


function khistoryService($http) {
...
function KhistoryCtrl(khistoryService, $scope, $filter) {
  ...
  */
  $scope.applyDateRange = function(ev, picker) {
    ...
    url = '/khistory/api?' + queryUrl;
    console.log('url: ' + url);

    khistoryService.get(url)
    ...
    url = '/khistory/aggregate?' + queryUrl;
    khistoryService.get(url)
  ...
  khistoryService.get('/khistory/api/event')
    .then(function(output) {

      $scope.events = output.results;
      $scope.eventCount = output.count;
      $scope.pages = output;
      $scope.pages.results = undefined;
...
khistoryApp
  .controller('KhistoryController', KhistoryCtrl)
  .service('khistoryService', khistoryService);


khistoryApp.config(function($routeProvider) {
    $routeProvider
    .when("/", {
        templateUrl : "/static/shared/khistory.html",
        controller: "KhistoryController"
    })
   .otherwise({
      redirectTo: '/'
    });
